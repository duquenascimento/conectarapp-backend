...
export const getCombination = async (restaurantId: string): Promise<any> => {
  const combinationRoute = await apiDbConectar.callApi(`/system/combinacao/${restaurantId}`, 'GET')
...
export const putCombination = async (combinationId: string, body: CombinacaoInput) => {
  return await apiDbConectar.callApi(`/system/combinacao/${combinationId}`, 'PUT', JSON.stringify(body))
}
