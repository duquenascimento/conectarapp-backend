...
export const formatDataToBolecode = async (
  data: ConfirmOrderRequest,
  yourNumber: string,
  ourNumber: string,
  orderId: string,
  deliveryDate: DateTime,
): Promise<BoletoInter | DadoBoleto> => {
  ...
  if (bankClient === 'ITAU') {
    const itauData = {
      ...
      codigo_carteira: '109',
      valor_total_titulo: data.supplier.discount.orderValueFinish
        .toFixed(2)
        .replace('.', '')
        .padStart(12, '0'),
      codigo_especie: '01',
      ...
      pagador: {
        pessoa: {
          ...
          tipo_pessoa: {
            codigo_tipo_pessoa:
              data.restaurant.restaurant.companyRegistrationNumber > 11 ? 'J' : 'F',
      ...
      dados_individuais_boleto: [
        {
          ...
          valor_titulo: data.supplier.discount.orderValueFinish
            .toFixed(2)
            .replace('.', '')
            .padStart(12, '0'),
        ...
        data_juros:
          DateTime.fromISO(getPaymentDate(data.restaurant.restaurant.paymentWay as string))
            .set({
              day:
                DateTime.fromISO(
                  getPaymentDate(data.restaurant.restaurant.paymentWay as string),
                ).get('day') + 1,
            })
            .toISODate() ?? '',
        codigo_tipo_juros: '93',
        valor_juros: ((data.supplier.discount.orderValueFinish * 0.01) / 30)
          .toFixed(2)
          .replace('.', '')
          .padStart(17, '0'),
      },
    ...
    if (itauData.pagador.pessoa.tipo_pessoa.codigo_tipo_pessoa === 'J') {
      itauData.pagador.pessoa.tipo_pessoa.numero_cadastro_nacional_pessoa_juridica =
        data.restaurant.restaurant.companyRegistrationNumber;
    } else {
      itauData.pagador.pessoa.tipo_pessoa.numero_cadastro_pessoa_fisica =
        data.restaurant.restaurant.companyRegistrationNumber;
    }
    return itauData;
  }
