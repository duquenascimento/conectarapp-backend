...
import { type FornecedorMotor, type ProdutoCesta, type CombinacaoAPI, type CombinationResponse, type MotorCombinacaoRequest, type MotorCombinacaoResponse, type MotorCombinacaoWithSupplierNames, type PreferenciaClasse, type PreferenciaProduto } from '../types/quotationTypes'
...
export async function solveCombinations(prices: any[], products: ProdutoCesta[], restaurant: any): Promise<CombinationResponse[] | undefined> {
  ...
  for (const combination of combinations) {
    ...
    let suppliers: FornecedorMotor[] = reqSuppliers.filter((sup) => !combination.fornecedores_bloqueados.includes(sup.id))
