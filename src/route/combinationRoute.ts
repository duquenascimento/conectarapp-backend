...
export const combinationRoute = async (server: FastifyInstance): Promise<void> => {
  ...
  server.get('/combination/get-divide-suppliers-options', async (req, res): Promise<void> => {
    try {
      return await res.status(200).send({
        status: 200,
        data: [{label: '2 fornecedores', value: 2}] // Retornar um array com as opções disponíveis com a estrutura {label: string, value: number}
      })
    } catch (err) {
      const message = (err as Error).message
      if (message === process.env.INTERNAL_ERROR_MSG) {
